...
class SafeMachineTest:
    ...
    def test_keep_alive(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Waiting 8 seconds to test keep-alive (firmware timeout is 5s)...")
            ...
            for i in range(8):
                ...
                if not self.controller.is_connected():
                    ...
                    self.log_result("Keep-alive", False, f"Connection lost after {elapsed:.1f}s")
                    return False
        ...
        for test_name, command in safe_commands.items():
            try:
                ...
                if success:
                    ...
                    self.log_result(f"Command: {command}", True, 
    ...
    def test_file_operations(self) -> bool:
        ...
        # Test file creation
        try:
            test_content = f"# Test file created by CNC library test\n# Time: {datetime.now()}\n# This file is safe to delete\n"
            
            # Note: File upload would require XMODEM, which is complex for this test
            # Instead, we'll test file listing and other safe operations
        ...
        for test_name, query in queries.items():
            try:
                ...
                if success:
                    ...
                    self.log_result(f"Query: {query}", True, 
                                  f"Sent, got {len(messages)} responses")
    ...
    def test_spindle_operations(self) -> bool:
        ...
        try:
            ...
            self.logger.info("Testing spindle: Start at 1000 RPM, 2s dwell, then stop...")
            for cmd in spindle_commands:
                success = self.controller.send_command(cmd)
                if success:
                    if "G4 P2" in cmd:
                        self.logger.info("Waiting 2 seconds for spindle to reach speed...")
            ...
            self.log_result("Spindle Operations", True, "All operations completed safely")
    ...
    def test_gcode_validation(self) -> bool:
        ...
        for gcode in test_gcodes:
            try:
                ...
                self.log_result(f"G-code: {gcode}", True, 
    ...
    def test_machine_discovery(self) -> bool:
        ...
        try:
            ...
            if machines:
                self.log_result("Machine Discovery", True, 
                              f"Found {len(machines)} machine(s)")
                for machine in machines:
                    self.logger.info(f"  - {machine['machine']} at {machine['ip']}:{machine['port']}")
            else:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Test CNC Controller Library against real machine")
    parser.add_argument("--ip", required=True, help="Machine IP address")
    parser.add_argument("--port", type=int, default=2222, help="Machine port (default: 2222)")
