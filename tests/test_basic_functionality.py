...
def test_method_signatures():
    ...
    try:
        ...
        try:
            ...
            print("    ✗ send_command with wait_for_response - should have raised CommandError")
        except Exception as e:
            if "Not connected" in str(e):
                print("    ✓ send_command with wait_for_response - correctly raises CommandError")
            else:
                print(f"    ✗ send_command with wait_for_response - unexpected error: {e}")
        ...
        try:
            ...
            print("    ✗ execute_gcode with wait_for_ok - should have raised CommandError")
        except Exception as e:
            if "Not connected" in str(e):
                print("    ✓ execute_gcode with wait_for_ok - correctly raises CommandError")
            else:
                print(f"    ✗ execute_gcode with wait_for_ok - unexpected error: {e}")
        
        try:
            controller.execute_gcode("G28", wait_for_ok=False)
            print("    ✗ execute_gcode with wait_for_ok=False - should have raised CommandError")
        except Exception as e:
            if "Not connected" in str(e):
                print("    ✓ execute_gcode with wait_for_ok=False - correctly raises CommandError")
            else:
                print(f"    ✗ execute_gcode with wait_for_ok=False - unexpected error: {e}")
        ...
        try:
            ...
            if "Not connected" in str(e):
                print("    ✓ execute_gcode with timeout - correctly raises CommandError")
            else:
    ...
    for attr_name, expected_type in attributes:
        if hasattr(controller, attr_name):
            ...
            if expected_type == 'threading.Event':
                ...
                print(f"  ✓ {attr_name} - correctly initialized as {expected_type.__name__}")
            else:
                print(f"  ✗ {attr_name} - expected {expected_type.__name__}, got {type(attr_value).__name__}")
                all_passed = False
        else:
